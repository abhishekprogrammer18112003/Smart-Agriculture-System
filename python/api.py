...
def predict_motor_state(data: SensorData):
    try:
        ...
        if -1 in [plant_type_encoded, moisture_1_encoded, moisture_2_encoded, moisture_3_encoded]:
            return {"error": "Invalid input values"}

        # Create DataFrame for model prediction
        input_data = pd.DataFrame([[plant_type_encoded, moisture_1_encoded, moisture_2_encoded, moisture_3_encoded, data.humidity, data.temperature]],
                                  columns=["Plant Type", "Moisture 1", "Moisture 2", "Moisture 3", "Humidity", "Temperature"])
